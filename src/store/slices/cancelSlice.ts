...
import type { CancelLogEntry, CancelSource, CancelAuthMeta, SessionRecord } from '../../types';
...
function isRecordVoidable(record: SessionRecord, shiftStart: number | null): boolean {
...
export const createCancelSlice: StateCreator<
  ...
  [],
  [],
  Pick<AppStore, 'cancelLog' | 'cancelOpenTableItem' | 'voidHistoryItem'>
> = (set, get) => {
  ...
  return {
    ...
    cancelOpenTableItem: (tableId, orderItemId, meta) => {
      ...
      set((state) => ({
        tables: state.tables.map((tb) => {
          ...
          return {
            ...
            currentSession: {
              ...tb.currentSession,
              barOrders: tb.currentSession.barOrders.filter((o) => o.id !== orderItemId),
              totalBarCost: Math.max(0, tb.currentSession.totalBarCost - amount),
            },
          };
        }),
      }));
      ...
      get().addToast(
        'success',
        translate(get().settings.language, 'cancel.done_toast', { name: item.menuItemName }),
      );
      return true;
    },
    ...
    voidHistoryItem: (recordId, orderItemId, meta) => {
      ...
      if (!isRecordVoidable(record, shiftStart)) {
        get().addToast('error', translate(get().settings.language, 'cancel.too_old'));
        return false;
      }
      ...
      set((state) => {
        const sessionHistory = state.sessionHistory
          ...
          .filter((r) => !(r.id === recordId && r.tableCost <= 0 && (r.barOrders?.length ?? 0) === 0));
      ...
      const source: CancelSource = record.tableId === 0 ? 'quick-sale' : 'closed-session';
      appendCancelLog({
        itemName: item.menuItemName,
        quantity: item.quantity,
        amount,
