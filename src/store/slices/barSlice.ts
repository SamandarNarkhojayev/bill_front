...
import type { BarOrder, BarOrderItem, SessionRecord, InventoryRevision } from '../../types';
...
export const createBarSlice: StateCreator<AppStore, [], [], Pick<AppStore, 'barMenu' | 'barCategories' | 'barOrders' | 'inventoryRevisions' | 'addMenuItem' | 'updateMenuItem' | 'removeMenuItem' | 'addBarCategory' | 'updateBarCategory' | 'removeBarCategory' | 'addBarOrderToTable' | 'createBarOrder' | 'sellFromBar' | 'updateStock' | 'setStock' | 'createRevision'>> = (set, get) => ({
      ...
      createRevision: (items, notes) => {
        ...
        get().addToast('success', translate(get().settings.language, 'toasts.revision_saved'));
      },
