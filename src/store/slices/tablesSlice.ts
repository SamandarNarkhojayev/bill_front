...
> = (set, get) => ({
  ...
  startSession: (tableId, mode, options = {}) => {
    ...
    if (
      ...
      plannedDuration = estimateDurationSecondsForAmount(
        Date.now(),
        amount,
        pricePerHour,
        priceSchedule,
    ...
    const normalizedPackagePrice =
      typeof packagePrice === "number" && packagePrice > 0
        ? packagePrice
        : null;
    const normalizedTariffName =
      typeof tariffName === "string" && tariffName.trim()
        ? tariffName.trim()
        : null;
    ...
    if (updatedTable && window.electronAPI?.arduino) {
      window.electronAPI.arduino
        .setRelay(updatedTable.relayNumber, true)
        .catch(console.error);
    }
    ...
    const modeKey =
      mode === "time"
        ? "dashboard.mode_time"
        : mode === "amount"
          ? "dashboard.mode_amount"
          : "dashboard.mode_unlimited";
    get().addToast(
      "success",
      translate(lang, "dashboard.started", {
        name: tableName,
        mode: translate(lang, modeKey),
      }),
    );
  ...
  endSession: (tableId, endTimeOverride, payment = "cash") => {
    const table = get().tables.find((t) => t.id === tableId);
    ...
    const barCost = session.barOrders.reduce(
    ...
    if (get().settings.autoLightOff && window.electronAPI?.arduino) {
      window.electronAPI.arduino
        .setRelay(table.relayNumber, false)
        .catch(console.error);
    }
    ...
    get().addToast(
      "info",
      translate(get().settings.language, "dashboard.ended_toast", {
        name: table.name,
        sum: (tableCost + barCost).toLocaleString(),
  ...
  pauseSession: (tableId) => {
    const table = get().tables.find((t) => t.id === tableId);
    if (!table || !table.currentSession || table.currentSession.pausedAt)
      return;
        ...
        t.id === tableId && t.currentSession
          ? {
              ...
              currentSession: {
                ...
                pausedAt: pauseStart,
                pauseIntervals: [
                  ...(t.currentSession.pauseIntervals ?? []),
    ...
    get().addToast(
      "info",
      translate(get().settings.language, "dashboard.paused_toast", {
        name: table.name,
      }),
    );
  },
  ...
  resumeSession: (tableId) => {
    ...
    if (window.electronAPI?.arduino) {
      window.electronAPI.arduino
        .setRelay(table.relayNumber, true)
        .catch(console.error);
    }
    get().addToast(
      "success",
      translate(get().settings.language, "dashboard.resumed_toast", {
        name: table.name,
      }),
    );
  },
  ...
  toggleLight: (tableId) => {
    ...
    if (window.electronAPI?.arduino) {
      window.electronAPI.arduino
        .setRelay(table.relayNumber, newState)
        .catch(console.error);
    }
  },
  ...
  syncTablesFromArduino: (relayCount, relays) => {
    ...
    set((state) => {
      const newTables = relays.map((relay) => {
        ...
        const settingsTable = state.settings.tables.find(
          (t) => t.relayNumber === relay.number,
        );
        return {
          id: relay.number,
          ...
          lightOn: relay.state,
          pricePerHour:
            settingsTable?.pricePerHour || state.settings.defaultPricePerHour,
      ...
      const newSettingsTables = newTables.map((t) => {
        const existing = state.settings.tables.find(
          (st) => st.relayNumber === t.relayNumber,
        );
        return {
          id: t.id,
          name: t.name,
    ...
    get().addToast(
      "info",
      translate(get().settings.language, "toasts.relays_detected", {
        count: relayCount,
      }),
    );
  },
  ...
  restoreLightsToArduino: () => {
    ...
    tables.forEach((table) => {
      ...
      if (shouldBeOn) {
        restoreCandidates++;
        window
          .electronAPI!.arduino!.setRelay(table.relayNumber, true)
          .then(() => {
            console.log(
              `[Arduino] ✅ Restored light for ${table.name} (relay ${table.relayNumber})`,
            );
          ...
          .catch((err) => {
            console.error(
              `[Arduino] ❌ Failed to restore light for ${table.name}:`,
              err,
            );
          });
        ...
        window
          .electronAPI!.arduino!.setRelay(table.relayNumber, false)
          .catch(() => {});
        set((state) => ({
          tables: state.tables.map((t) =>
            t.id === table.id ? { ...t, lightOn: false } : t,
          ),
        }));
      }
    });
    ...
    if (restoreCandidates > 0) {
      ...
      setTimeout(() => {
        get().addToast(
          "success",
          translate(get().settings.language, "toasts.lights_restored", {
            count: restoreCandidates,
          }),
        );
      }, 500);
    }
  ...
  addReservation: (
    ...
    get().addToast(
      "success",
      translate(get().settings.language, "toasts.table_reserved", {
        name: tableName,
      }),
    );
  },
  ...
  cancelReservation: (reservationId) => {
    ...
    get().addToast(
      "info",
      translate(get().settings.language, "toasts.reservation_cancelled"),
    );
  },
});
