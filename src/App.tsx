...
telegram.setDataProvider(() => {
  ...
  const tables = s.tables.map((t) => {
    if (t.status === "occupied" && t.currentSession) {
      ...
      const barCost = ses.barOrders.reduce(
      ...
      return {
        ...
        status: t.status,
        elapsedMin,
        currentCost: Math.round(tableCost + barCost),
        paused: !!ses.pausedAt,
      };
    }
  ...
  if (s.currentShift) {
    ...
    const sessions = s.sessionHistory.filter(
      (x) => x.startTime >= sh.startTime,
    );
    const tableRevenue = sessions.reduce(
      (sum, x) => sum + (x.tableCost || 0),
      0,
    );
  ...
  const week = aggregate(
    s.sessionHistory.filter((x) => x.startTime >= weekStart),
  );
  const month = aggregate(
    s.sessionHistory.filter((x) => x.startTime >= monthStart),
  );
  ...
  const perTable = Array.from(perTableMap.entries()).map(([name, v]) => ({
    name,
    ...v,
  }));
...
function App() {
  ...
  const [whatsNewEntries, setWhatsNewEntries] = useState<
    ReturnType<typeof getUnseenEntries>
  >([]);
  ...
  useEffect(() => {
    ...
    const entries = getUnseenEntries(
    ...
    const unsubCmd = cloudSync.onCommand(async (cmd) => {
      ...
      if (cmd.type === "TABLE_TOGGLE_LIGHT") {
        ...
        const fresh = useStore
          .getState()
          .tables.find((t) => t.id === cmd.tableId);
      ...
      if (cmd.type === "TABLE_END_SESSION") {
        if (table.status !== "occupied")
          return { ok: false, error: "NOT_OCCUPIED" };
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      store.tables.forEach((table) => {
        ...
        if (
          (session.mode === "time" || session.mode === "amount") &&
          ...
          if (
            session.plannedDuration - elapsedSec <= 0 &&
            !handled.has(table.id)
          ) {
    ...
    switch (currentPage) {
      ...
      case "bar":
        return (
          <BarPage
            key="barpage"
            department={settings.kitchenSeparate ? "bar" : "combined"}
          />
        );
      case "kitchen":
        return (
          <BarPage
            key="barpage"
            department={settings.kitchenSeparate ? "kitchen" : "combined"}
          />
        );
      case "reports":
        return <ReportsPage />;
      ...
      default:
        return <Dashboard />;
      case "tournaments":
        return canAccessTournamentAndTariffPages ? (
          <TournamentPage />
        ) : (
          <Dashboard />
        );
  ...
  return (
    <div
      className={`app ${settings.theme === "light" ? "theme-light" : ""} ${sidebarCollapsed ? "sidebar-collapsed" : ""}`}
    >
      <Sidebar />
