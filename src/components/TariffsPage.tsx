...
const getRuleSegments = (
  rule: Pick<TablePriceRule, "startTime" | "endTime">,
...
const rulesOverlap = (
  left: Pick<TablePriceRule, "startTime" | "endTime">,
  right: Pick<TablePriceRule, "startTime" | "endTime">,
): boolean => {
...
const findPriceScheduleConflict = (
  rules: TablePriceRule[],
): { firstRule: TablePriceRule; secondRule: TablePriceRule } | null => {
...
const getPriceScheduleConflict = (
  rules: TablePriceRule[],
): PriceScheduleConflict | null => {
...
const TariffsPage: React.FC = () => {
  const {
    ...
    useShallow((s) => ({
      ...
      addToast: s.addToast,
      tariffs: s.tariffs,
      addTariff: s.addTariff,
      updateTariff: s.updateTariff,
      removeTariff: s.removeTariff,
    })),
  );
  const { t } = useT();
  const [showCreateModal, setShowCreateModal] = useState(false);
  const [editingTariff, setEditingTariff] = useState<Tariff | null>(null);
  const canAccessAllSettings =
  ...
  const activeTables = useMemo(
    () => settings.tables.filter((t) => t.isActive),
    [settings.tables],
  );
  ...
  const getPriceScheduleConflictMessage = (
    rules: TablePriceRule[],
  ): string | null => {
  ...
  const updateTableSetting = (
    ...
    if (!canAccessAllSettings) {
      addToast("error", "Недостаточно прав для изменения настроек тарифов");
      return;
    }
  ...
  const updateTablePriceSchedule = (
    tableIndex: number,
    rules: TablePriceRule[],
  ) => {
    if (!canAccessAllSettings) {
  ...
  const addPriceRule = (tableIndex: number) => {
    ...
    const fallbackStart = lastRule
      ? parseTimeToMinutes(lastRule.endTime)
      : 9 * 60;
    const nextStartMinutes = Math.max(
      0,
      Math.min(DAY_MINUTES - 60, fallbackStart),
    );
  ...
  const handleApplyBulkPriceRule = () => {
    ...
    if (skippedTables.length > 0) {
      addToast(
        "warning",
        t("tariffs.bulkSkipped", { tables: skippedTables.join(", ") }),
      );
    }
  };
  ...
  const handleCreateOrUpdateTariff = () => {
    if (!canAccessAllSettings) {
      addToast("error", "Недостаточно прав для изменения тарифов");
      return;
    }
    if (!tariffName.trim()) {
  ...
  const getTotalProductsCost = (products: TariffMenuProduct[]) => {
    return products.reduce((sum, product) => {
      return (
        sum + (barMenuPriceById.get(product.productId) || 0) * product.quantity
      );
    }, 0);
  };
  ...
  return (
    <div className="page-content">
      ...
      {canAccessAllSettings && (
        ...
        >
          <div className="settings-section-header">
            ...
            >
              <Cpu size={14} />
              {t("tariffs.tablesAutoDetected", {
                count: settings.tables.length,
              })}
            </span>
          </div>
          <div
          ...
          {showBulkPriceEditor && (
            ...
            >
              ...
              >
                ...
                >
                  <span
                    style={{ fontSize: 13, fontWeight: 700, color: "#ddd6fe" }}
                  >
              ...
              >
                ...
                >
                  <input
                    ...
                    value={table.name}
                    onChange={(e) =>
                      updateTableSetting(index, "name", e.target.value)
                    }
                    className="form-input"
                  />
                  <span
                    className="settings-relay-badge"
                  ...
                  <NumberInput
                    ...
                    onChange={(n) =>
                      updateTableSetting(index, "pricePerHour", n)
                    }
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        {t("tariffs.intervalsCount", {
                          count: (table.priceSchedule || []).length,
                        })}
                      </span>
                    </div>
                    <button
              ...
              >
                ...
                >
                  <div>
                    ...
                    >
                      ●{" "}
                      {tariff.isActive
                        ? t("tariffs.statusActive")
                        : t("tariffs.statusInactive")}
                    </div>
                  </div>
                  {canAccessAllSettings && (
                ...
                >
                  ...
                  >
                    <Clock size={14} />
                    <span>
                      {t("tariffs.timeLabel")}: {tariff.startTime} -{" "}
                      {tariff.endTime}
                    </span>
                  </div>
                  <div
                  ...
                  >
                    <Package size={14} />
                    <span>
                      {t("tariffs.packageLabel")}: {tariff.durationHours}{" "}
                      {t("tariffs.hoursShort")}
                    </span>
                  </div>
                  <div
                    style={{
                  ...
                  >
                    <Tag size={14} />
                    <span>
                      {t("tariffs.priceLabel")}: {tariff.price}{" "}
                      {settings.currency}
                    </span>
                  </div>
                </div>
                ...
                {tariff.menuProducts.length > 0 && (
                  ...
                  >
                    ...
                    <div
                      style={{ fontSize: 11, color: "#8b5cf6", marginTop: 6 }}
                    >
                      {t("tariffs.extraValue")}: +{productsCost}{" "}
                      {settings.currency}
                    </div>
                  </div>
                )}
                ...
                >
                  <div style={{ fontSize: 13, color: "#64748b" }}>
                    {t("tariffs.tablesLabel")}:{" "}
                    {tariff.tableIds
                      .map((id) => activeTableNameById.get(id) || `#${id}`)
                      .join(", ")}
                  </div>
                  {canAccessAllSettings && (
                    <button
      ...
      {showCreateModal && canAccessAllSettings && (
        <div
          ...
          >
            ...
            >
              ...
              >
                ...
                <div className="settings-field">
                  <label className="settings-label">
                    {t("tariffs.tariffPrice")} ({settings.currency})
                  </label>
                  <NumberInput
                    value={price}
                    onChange={setPrice}
            ...
            <div className="modal-footer">
              ...
              <button
                onClick={handleCreateOrUpdateTariff}
                className="btn btn-primary"
              >
                <Plus size={16} />
