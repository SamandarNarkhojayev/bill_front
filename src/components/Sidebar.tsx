...
const Sidebar: React.FC = () => {
  ...
  const occupiedTables = useStore(
    (s) => s.tables.filter((tbl) => tbl.status === "occupied").length,
  );
  ...
  const sidebarLanguages = LANGUAGES.filter(
    (l) => l.code === "ru" || l.code === "kk",
  );
  ...
  const userForbiddenIds: PageType[] = ["tournaments", "tariffs"];
  const roleVisibleItems = isUser
    ? visibleMenuItems.filter(
        (item) =>
          !userForbiddenIds.includes(item.id) &&
          (item.id === "dashboard" || userAllowed.includes(item.id)),
      )
    : visibleMenuItems;
  ...
  const pinned =
    settings.sidebarPinned && settings.sidebarPinned.length > 0
      ? settings.sidebarPinned
      : ["dashboard", "bar", "reports", "settings"];
  const pinnedItems = isUser
    ? roleVisibleItems
    : roleVisibleItems.filter((item) => pinned.includes(item.id));
  const moreItems = isUser
    ? []
    : roleVisibleItems.filter((item) => !pinned.includes(item.id));
